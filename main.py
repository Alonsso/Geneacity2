...
def main():
    ...
    class Botones:
        ...
        def draw(self):
            # Draw the button and detect clicks
            distinguirBtn = False
    ...
    while running:
        ...
        if btnInicio.draw():
            geneacity.IniciarJuego()  # Start the game only when the button is clicked
            running = False
            print("Iniciar Juego")
...
if __name__ == "__main__":
    main()  # Ensure the menu is shown first by running main() only when the script is executed directly
